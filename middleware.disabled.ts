...
export async function middleware(request: NextRequest) {
  ...
  // TEMP: Debug logging
  console.log("[MW] IN", pathname, search);
  
  // Skip middleware for API routes and auth callback
  if (pathname.startsWith('/api') || pathname.startsWith('/auth')) {
    console.log("[MW] SKIP api/auth, calling updateSession");
    const result = await updateSession(request);
    console.log("[MW] NEXT (api/auth) status:", result.status);
    return result;
  }
  ...
  if (intlResponse.status === 307 || intlResponse.status === 308) {
    ...
    if (location) {
      // Preserve search params if next-intl dropped them
      const redirectUrl = new URL(location, request.url);
      if (search && !redirectUrl.search) {
        redirectUrl.search = search;
        console.log("[MW] REDIRECT (intl) to:", redirectUrl.pathname + redirectUrl.search, "(preserved search)");
      ...
      console.log("[MW] REDIRECT (intl) to:", redirectUrl.pathname + redirectUrl.search);
      return intlResponse;
    }
  }

  // Then handle auth with the request (locale is already set by next-intl)
  console.log("[MW] Calling updateSession");
  const authResponse = await updateSession(request);
  
  console.log("[MW] Auth status:", authResponse.status);
  
  // If auth middleware returned a redirect, preserve search params
  if (authResponse.status === 307 || authResponse.status === 308) {
    const location = authResponse.headers.get('location');
    console.log("[MW] Auth wants redirect to:", location);
    if (location) {
      const redirectUrl = new URL(location, request.url);
      if (search && !redirectUrl.search) {
        redirectUrl.search = search;
      ...
      console.log("[MW] REDIRECT (auth) to:", redirectUrl.pathname + redirectUrl.search);
      return NextResponse.redirect(redirectUrl, { status: authResponse.status });
    }
    console.log("[MW] RETURN (auth redirect, no location)");
    return authResponse;
  }

  // Merge intl response (locale cookies, etc.) with auth response
  console.log("[MW] NEXT (continue)");
